# 營業額提交系統

市場擺攤商家的營業額提交與查詢系統。

## 功能

- 📱 手機號碼快速登入
- 💰 提交營業額（含租金自動計算、成本記錄）
- 📊 查看營業紀錄
- 📈 管理員統計報表

---

## 快速開始

### 1. 設定 Google Sheets

#### 1.1 建立試算表

1. 前往 [Google Sheets](https://sheets.google.com) 建立新試算表
2. 記下網址中的 **Spreadsheet ID**（網址格式：`https://docs.google.com/spreadsheets/d/{SPREADSHEET_ID}/edit`）

#### 1.2 建立資料表

在試算表中建立以下 3 個 Sheet（點擊底部 `+` 新增）：

| Sheet 名稱 | 用途 |
|-----------|------|
| `users` | 使用者資料 |
| `markets` | 市場資料 |
| `revenues` | 營業額紀錄 |

#### 1.3 填入欄位標題

**Sheet: `users`**
```
名稱	手機	權限	狀態	建立時間	id
```

**Sheet: `markets`**
```
名稱	租金規則	狀態	id
```

> 租金規則格式: `1-5:400,6-7:600`（1=週一...7=週日，詳見 `seed_data.md`）

**Sheet: `revenues`**
```
日期	市場	營業額	租金	停車費	清潔費	其他成本	淨利	提交者	備註	提交時間	id	market_id	submitted_by_phone
```

> 💡 **提示**：可直接複製 `seed_data.md` 中的測試資料貼入各 Sheet

> ⚠️ **重要**：`手機` 和 `submitted_by_phone` 欄位需設為「純文字」格式，避免手機號碼前導 0 被移除

---

### 2. 部署 Google Apps Script

#### 2.1 開啟 Apps Script

在 Google Sheets 中：
1. 點擊 **擴充功能** → **Apps Script**
2. 會開啟新的 Apps Script 編輯器視窗

#### 2.2 貼上程式碼

1. 刪除預設的 `function myFunction() {}` 
2. 複製 `gas/Code.gs` 的全部內容貼上
3. **修改第 11 行**，將 `YOUR_SPREADSHEET_ID` 替換成你的 Spreadsheet ID：

```javascript
const SPREADSHEET_ID = 'YOUR_SPREADSHEET_ID'; // ← 改這裡
```

4. 按 `Ctrl + S` 儲存

#### 2.3 部署

1. 點擊右上角 **部署** → **新增部署**
2. 點擊齒輪圖示，選擇 **網頁應用程式**
3. 設定：
   - **說明**：營業額系統 API
   - **執行身份**：我
   - **存取權限**：**任何人**
4. 點擊 **部署**
5. 首次部署需要授權，點擊 **授權存取** → 選擇你的 Google 帳號 → **允許**
6. 複製顯示的 **網頁應用程式網址**（格式：`https://script.google.com/macros/s/xxx/exec`）

> ⚠️ **注意**：每次修改 Code.gs 後，需要重新部署才會生效。點擊 **部署** → **管理部署** → **編輯** → 版本選「新版」→ **部署**

---

### 3. 設定前端

#### 3.1 安裝依賴

```bash
cd app
npm install
```

#### 3.2 設定環境變數

複製環境變數範例檔：

```bash
cp .env.example .env
```

編輯 `.env`，填入你的 GAS URL：

```
VITE_GOOGLE_SCRIPT_URL=https://script.google.com/macros/s/你的部署ID/exec
```

#### 3.3 啟動開發伺服器

```bash
npm run dev
```

瀏覽器開啟 http://localhost:5173/mtz/

---

### 4. 測試

使用 `seed_data.md` 中的測試資料：

| 手機號碼 | 姓名 | 權限 | 預期結果 |
|---------|------|------|---------|
| 0912345678 | 王小明 | admin | 可看全部紀錄 + 統計 |
| 0923456789 | 李小華 | user | 只能看當日自己的 |
| 0945678901 | 陳大偉 | user | 登入失敗（已停用） |
| 0999999999 | - | - | 登入失敗（未註冊） |

---

## 部署到 GitHub Pages

### 1. 建立 GitHub Repository

1. 前往 [GitHub](https://github.com) 建立新 repo，名稱為 `mtz`
2. 不要勾選任何初始化選項

### 2. 設定 Secrets

在 GitHub repo 中：
1. 前往 **Settings** → **Secrets and variables** → **Actions**
2. 點擊 **New repository secret**
3. 新增：
   - **Name**: `VITE_GOOGLE_SCRIPT_URL`
   - **Value**: 你的 GAS 網址

### 3. 設定 GitHub Pages

1. 前往 **Settings** → **Pages**
2. **Source** 選擇 **GitHub Actions**

### 4. Push 程式碼

在專案根目錄執行：

```bash
git init
git add .
git commit -m "Initial commit"
git branch -M main
git remote add origin https://github.com/你的帳號/mtz.git
git push -u origin main
```

Push 後會自動觸發部署，完成後可在以下網址存取：

```
https://你的帳號.github.io/mtz/
```

---

## 專案結構

```
mtz/
├── app/                          # Vue 前端
│   ├── src/
│   │   ├── assets/main.css       # 全域樣式
│   │   ├── components/           # 共用組件
│   │   │   ├── LoadingSpinner.vue
│   │   │   ├── PhoneInput.vue
│   │   │   ├── RevenueFilters.vue
│   │   │   ├── RevenueForm.vue
│   │   │   ├── RevenueList.vue
│   │   │   └── StatsSummary.vue
│   │   ├── composables/          # 組合函數
│   │   │   ├── useApi.ts
│   │   │   └── useAuth.ts
│   │   ├── router/index.ts       # 路由設定
│   │   ├── types/index.ts        # TypeScript 型別
│   │   └── views/                # 頁面
│   │       ├── LoginView.vue
│   │       ├── SubmitView.vue
│   │       └── HistoryView.vue
│   ├── public/
│   │   └── 404.html              # GitHub Pages SPA 路由支援
│   ├── .env.example
│   ├── package.json
│   └── vite.config.ts
├── gas/
│   └── Code.gs                   # Google Apps Script 後端
├── .github/workflows/
│   └── deploy.yml                # GitHub Actions 部署
├── spec_optimized.md             # 技術規格書
└── seed_data.md                  # 測試資料
```

---

## 常見問題

### Q: 登入時出現「網路連線錯誤」

**可能原因**：
1. GAS 尚未部署或網址錯誤
2. `.env` 中的網址不正確

**解決方式**：
1. 確認 GAS 已成功部署
2. 確認 `.env` 中的網址正確（需包含 `/exec` 結尾）
3. 重新啟動 dev server

### Q: 登入時出現「此手機號碼未註冊」

**可能原因**：Google Sheet 中沒有該手機號碼

**解決方式**：在 `users` Sheet 中新增使用者資料

### Q: 篩選提交者時查不到資料

**可能原因**：手機號碼格式問題（前導 0 被移除）

**解決方式**：
1. 選取 `手機` 和 `submitted_by_phone` 欄位
2. 格式 → 數字 → 純文字
3. 重新貼上資料

### Q: 部署後頁面空白

**可能原因**：GitHub Secrets 未設定或設定錯誤

**解決方式**：
1. 確認 `VITE_GOOGLE_SCRIPT_URL` Secret 已設定
2. 重新執行 GitHub Actions（Actions → 選擇 workflow → Re-run）

### Q: 修改 GAS 程式碼後沒有生效

**原因**：需要重新部署

**解決方式**：
1. Apps Script 編輯器 → 部署 → 管理部署
2. 編輯現有部署 → 版本選「新版」→ 部署

---

## 技術棧

- **前端**：Vue 3 + TypeScript + Vite
- **UI**：PrimeVue + Tailwind CSS
- **後端**：Google Apps Script
- **資料庫**：Google Sheets
- **部署**：GitHub Pages + GitHub Actions
